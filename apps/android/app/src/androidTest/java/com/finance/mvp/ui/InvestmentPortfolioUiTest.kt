...
class InvestmentPortfolioUiTest {
    ...
    @Test fun showsTwoSberAccountsAndCombinedPortfolio() {
        ...
        compose.onNodeWithTag("refresh-investment-recommendations").performScrollTo().assertIsDisplayed()
    }
    ...
    @Test fun showsReportAssumptionsWhenRecommendationIsExpanded() {
        ...
        compose.onNodeWithText("Допущения").performScrollTo().assertIsDisplayed()
        compose.onNodeWithText("Горизонт: 5 лет").performScrollTo().assertIsDisplayed()
    }
    ...
    @Test fun showsRestoredHistorySummaryAndAccountProfiles() {
        ...
        compose.onNodeWithText("Краткий итог").performScrollTo().assertIsDisplayed()
        compose.onNodeWithText("Портфель соответствует профилю риска").performScrollTo().assertIsDisplayed()
    }
}
