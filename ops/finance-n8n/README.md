...
Текущий backend отклоняет повторный terminal callback, даже с новым nonce. При потерянном ответе шлюз сохраняет зашифрованный результат в `delivery_failed` для сверки с backend и не генерирует рекомендацию заново. Для полностью автоматического восстановления потребуется идемпотентный callback/подтверждение результата на стороне backend. Это не меняется данной веткой.
