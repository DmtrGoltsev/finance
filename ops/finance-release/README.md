...
После gate стадия `stage` сначала проверяет владельцев каталогов и обеих
существующих ролей/БД; конфликт останавливает запуск до изменения хоста.
Каждый запуск создаёт **новый** дамп текущей Finance DB с отдельным именем,
SHA-256 и пробным восстановлением в отдельную БД: старый дамп не переиспользуется.
Исходный release, в том числе исполняемые через `sudo` Python-файлы и их
родительские каталоги, остаётся root-owned без права записи для `finance`.
`npm ci` работает от `finance` во временном каталоге; зависимости затем
переносятся в release и закрепляются за root. Перед активацией проверяются
владелец, режим и SHA-256 исходных файлов. Далее стадия заводит отдельные
роли/БД `finance_n8n` и `finance_analysis`,
