...
Reserved post-MVP `sourceType` vocabulary such as `file_import`, `bank_api`, `sms` and `push` remains rejected by direct transaction create/update flows. Capture drafts must use a separate user-confirmed lifecycle: user-initiated OCR from a selected screenshot, local/on-device before structured draft review, no SMS/push/notification interception, no raw SMS/push/notification body server-side, structured draft with `idempotencyKey`/`evidenceHash`, and transaction creation only after user confirm/edit.
