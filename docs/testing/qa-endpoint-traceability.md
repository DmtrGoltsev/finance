...
Широкие строки ниже про users/profile, households, invites, memberships, exports, deletion/leave privacy flows, password reset, import/bank/full SMS/push/broker integrations и debug/support остаются traceability backlog/post-MVP release gates. Safe auto-capture является отдельной post-MVP draft-only surface: она не публикует raw SMS/notification body на backend и не создает transaction без user confirm/edit.
