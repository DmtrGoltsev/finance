...
Широкие строки ниже про users/profile, households, invites, memberships, exports, deletion/leave privacy flows, password reset, import/bank/full SMS/push/broker integrations и debug/support остаются traceability backlog/post-MVP release gates. Capture drafts являются отдельной draft-only surface: OCR запускается пользователем из выбранного скриншота локально/on-device, SMS/push/notifications не перехватываются, raw SMS/push/notification body не публикуется на backend, transaction не создается без user confirm/edit.
