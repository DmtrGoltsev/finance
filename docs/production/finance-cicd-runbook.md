...
- `push` to branch names containing `release`: CI, tests, builds, packaging, and
  artifact upload only.
- `workflow_dispatch`: same CI/package lanes, plus optional approved production
  deploy actions controlled by explicit inputs and the GitHub `production`
  environment.
...
Post-deploy checks run on the host:
...
Migrations are part of the backend deploy path and are disabled by default. See
`docs/production/finance-db-migrations.md`.

The workflow runs Alembic only when all are true:
...
- Migrations require exact revision inputs and backup evidence.
...
- backup proof, if migrations were run
